...
if(!($arParams['CHECK_RIGHTS'] == 'N' || $USER->CanDoOperation('edit_own_profile')) || $arResult["ID"]<=0):?>
	<div class='container pt-72 pb-72 contact-form-modal__body' >
		<h4 class='text-center mb-72' style='color:red;'>Вам нужно войти или зарегестрироваться, чтобы получить доступ к этой странице!</h4>
...
<div class="bx-auth-profile container pt-72 pb-72">

<h1>
	Профиль пользователя <?=$arResult["arUser"]["LOGIN"]?>
</h1>
...
<p class='text-center'><input type="submit" class="btn btn--lg btn--color" name="code_submit_button" value="<?echo GetMessage("main_profile_send")?>" /></p>
...
<table class="data-table profile-table">
	...
	<?endif;?>
	<p class='mt-72'><input class="btn btn--lg btn--color" type="submit" name="save" value="<?=(($arResult["ID"]>0) ? GetMessage("MAIN_SAVE") : GetMessage("MAIN_ADD"))?>">&nbsp;&nbsp;<input class="btn btn--lg btn--color" type="reset" value="<?=GetMessage('MAIN_RESET');?>"></p>
</form>
